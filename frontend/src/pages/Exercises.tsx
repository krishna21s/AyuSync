...
import {
  Dumbbell, Play, Clock, Flame, X, Bot,
    ...
    <div className="flex flex-col items-center gap-3">
      ...
      <button
        ...
        className={`px-6 py-2.5 rounded-xl font-semibold text-sm flex items-center gap-2 active:scale-95 transition-all ${
          running ? "bg-gray-900 text-white" : "bg-primary text-primary-foreground"
        }`}
      >
        {running ? (
...
const Exercises = () => {
  ...
  const { exercises, progress, isLoading, completeExercise, isDone } = useExercises(
    tab === "all" ? undefined : tab
  );

  const progressPct = progress?.progress_pct || 0;
  const totalDuration = progress?.duration_total || 0;

  const handleComplete = useCallback(async (exId: number, duration: number) => {
    await completeExercise(exId, duration);
  }, [completeExercise]);

  const parseSteps = (steps: string | null | undefined): string[] => {
    if (!steps) return [];
          ...
          <div className="flex-1 flex flex-col min-w-0">
            ...
            <main className="flex-1 p-4 md:p-6 pb-24 md:pb-6 overflow-y-auto flex flex-col items-center justify-center">
              <div className="text-center animate-pulse">
                ...
                <h3 className="text-lg font-bold text-gray-900 mb-2">Generating personalized exercises...</h3>
                <p className="text-sm text-gray-500">AI is curating a gentle physical routine based on your medical profile.</p>
              </div>
            </main>
          </div>
          <BottomNav />
            ...
            <div className="max-w-5xl mx-auto">
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }} className="card-dark p-5 mb-6">
                ...
                <div className="flex justify-between mt-2">
                  <span className="text-[11px] text-white/40">{progress?.completed || 0}/{progress?.total || 0} {t("routine.completed")}</span>
                ...
                {tabKeys.map((tb) => (
                  <button
                    ...
                    className={`px-4 py-2.5 rounded-xl text-xs font-semibold whitespace-nowrap transition-all ${
                      tab === tb.key ? "bg-gray-900 text-white" : "bg-white border border-gray-200 text-gray-500 hover:bg-gray-50"
                    }`}
                  >
                    {t(tb.tKey)}
                  </button>
              ...
              {phaseConfig.map((phase, pi) => {
                const phaseItems = exercises.filter((e: any) => e.phase === phase.key);
                if (phaseItems.length === 0) return null;
                const PhaseIcon = phase.icon;
                return (
                  <motion.div key={phase.key} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 + pi * 0.1 }} className="mb-8">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                      {phaseItems.map((ex: any, i: number) => {
                        const done = isDone(ex.id);
                        return (
                          <motion.div key={ex.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 + i * 0.05 }}
                            whileHover={{ y: -4, scale: 1.01 }} onClick={() => setSelectedEx(ex)}
                            className={`card-white cursor-pointer overflow-hidden group transition-all ${done ? "opacity-60 ring-2 ring-primary/30" : ""}`}
                          >
                            <div className="relative h-44 overflow-hidden bg-gray-50">
                              <img src={ex.image_url || "/exercises/placeholder.png"} alt={ex.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                              {done && (
                              ...
                              <span className={`absolute top-3 right-3 text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-lg ${
                                ex.difficulty === "easy" ? "bg-green-100 text-green-700" : "bg-orange-100 text-orange-700"
                              }`}>
                                {t(ex.difficulty === "easy" ? "ex.easy" : "ex.medium")}
                              </span>
                            </div>
                            <div className="p-4">
                              <h4 className="font-bold text-sm text-gray-900 mb-1">{ex.name}</h4>
                              <p className="text-xs text-gray-400 mb-3 line-clamp-2">{ex.description}</p>
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                  <span className="text-[11px] text-gray-500 flex items-center gap-1">
                                    <Clock className="h-3 w-3" /> {ex.duration_seconds} {t("ex.sec")}
                                  </span>
                                  {ex.reps && (
                                    <span className="text-[11px] text-gray-500 flex items-center gap-1">
          ...
          >
            ...
            >
              <div className="relative h-52 overflow-hidden rounded-t-2xl bg-gray-50">
                <img src={selectedEx.image_url || "/exercises/placeholder.png"} alt={selectedEx.name} className="w-full h-full object-cover" />
                <button onClick={() => setSelectedEx(null)} className="absolute top-3 right-3 p-2 rounded-xl bg-white/90 backdrop-blur-sm hover:bg-white transition-colors">
                  <X className="h-4 w-4 text-gray-600" />
                </button>
                <span className={`absolute top-3 left-3 text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-lg ${
                  selectedEx.difficulty === "easy" ? "bg-green-100 text-green-700" : "bg-orange-100 text-orange-700"
                }`}>
              ...
              <div className="p-6">
                <h3 className="text-xl font-bold text-gray-900 mb-1">{selectedEx.name}</h3>
                <p className="text-sm text-gray-500 mb-4">{selectedEx.description}</p>
                ...
                <div className="flex items-center gap-3 mb-5">
                  <span className="text-xs text-gray-500 flex items-center gap-1 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">
                    <Clock className="h-3 w-3" /> {selectedEx.duration_seconds} {t("ex.sec")}
                  </span>
                  {selectedEx.reps && (
                    <span className="text-xs text-gray-500 flex items-center gap-1 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">
                  ...
                  <span className="text-xs text-gray-500 flex items-center gap-1 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">
                    <Target className="h-3 w-3" /> {selectedEx.muscle_group}
                  </span>
                </div>
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-2">
                    {parseSteps(selectedEx.steps).map((step: string, i: number) => (
                ...
                <div className="flex flex-col items-center pt-2 border-t border-gray-100">
                  ...
                  <WorkoutTimer
                    totalSeconds={selectedEx.duration_seconds}
                    onComplete={() => handleComplete(selectedEx.id, selectedEx.duration_seconds)}
                  />
                  {isDone(selectedEx.id) && (
